...
class Teris:
    def __init__(self):
        ...
        self.restartBut = Button(self.root, text="Greedy agent", height=2, width=13, font=(18), command=self.isGreedy)
        self.restartBut.place(x=340, y=300)
        ...
        self.menu.add_cascade(label='greedy agent', menu=self.isGreedy)
    ...
    def isGreedy(self):
        askGreedy = messagebox.askquestion("Greedy agent", "Using greedy agent?")
        if askGreedy == 'yes':
            self.greedy()
            self.restart()
        else:
